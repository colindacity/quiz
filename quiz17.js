...
function initQuiz() {
  ...
  function selectOption(optionIndex, questionType) {
    // Debounce selections to prevent rapid-fire clicks
    const now = Date.now();
    if (now - lastSelectedTime < 200) { // 200ms debounce
      return;
    }
    ...
    if (isTransitioning) {
      logDebug('Ignoring selection during transition');
      return;
    }
    ...
    if (optionIndex < 0 || optionIndex >= options.length) {
      logDebug(`Invalid option index: ${optionIndex}`, 'error');
      return;
    }
    ...
    if (questionType === 'single') {
      ...
      options.forEach(opt => {
        opt.classList.remove('pulse-animation', 'shake-animation');
      });
      ...
      // Move to the next question after animation completes
      setTimeout(() => {
        isTransitioning = false;
      ...
      targetOption.classList.remove('pulse-animation', 'shake-animation');
      ...
      if (selectedOptions.includes(optionIndex)) {
        ...
        targetOption.classList.add('selected', 'pulse-animation');
        targetOption.setAttribute('aria-selected', 'true');
        selectedOptions.push(optionIndex);
      } else {
        // Give feedback that 2 are already selected
        targetOption.classList.add('shake-animation');
        setTimeout(() => {
          targetOption.classList.remove('shake-animation');
        }, 500);
      ...
      const btn = ensureNextButtonExists();
      if (btn) {
        const isComplete = selectedOptions.length === 2;
        btn.disabled = !isComplete;
        btn.setAttribute('aria-disabled', !isComplete);
  ...
  function calculateScores() {
    ...
    try {
      ...
      if (!validateScores(scores)) {
        logDebug('Score validation failed - using defaults', 'error');
        return getDefaultScores();
      }
  ...
  function showResults() {
    ...
    if (!resultsContainer || !profileSummary) {
      logDebug('Results container or profile summary not found', 'error');
      showError('Cannot display results - missing required elements');
      return;
    }
    ...
    try {
      ...
      // Find top archetype
      const topArchetype = Object.entries(scores.archetypes)
        .sort(([,a], [,b]) => b - a)[0][0];
      ...
      // Find top superpower
      const topSuperpower = Object.entries(scores.superpowers)
        .sort(([,a], [,b]) => b - a)[0][0];

      // Find top culture fits
      const topProductCulture = Object.entries(scores.productCultures)
        .sort(([,a], [,b]) => b - a)[0][0];
      const topPMCulture = Object.entries(scores.pmCultures)
        .sort(([,a], [,b]) => b - a)[0][0];
          ...
          <div class="pros-cons-section">
            <div class="pros-cons">
              <div class="pros">
                <h4>Key Strengths</h4>
                <ul>
                  ${archetypeData.pros.map(pro => `<li>${pro}</li>`).join('')}
                </ul>
              </div>
              <div class="cons">
                <h4>Growth Areas</h4>
                <ul>
                  ${archetypeData.cons.map(con => `<li>${con}</li>`).join('')}
                </ul>
              </div>
            </div>
          </div>

          <div class="culture-fit-section">
            <h3>Ideal Work Environment</h3>
            <p><strong>Product Culture:</strong> ${productCultureDescriptions[topProductCulture]}</p>
            <p><strong>PM Role:</strong> ${pmCultureDescriptions[topPMCulture]}</p>
          </div>

          <div class="company-recommendations">
            <h3>Companies That Match Your Profile</h3>
            <p>${companyRecommendations[topArchetype].join(', ')}</p>
          </div>

          <div class="action-section">
            <h3>Next Steps</h3>
            <p>Based on your ${getProsString(topArchetype)}, consider roles that allow you to ${superpowerActions[topSuperpower]}.</p>
          </div>
      ...
      // Submit results data
      submitQuizData(scores, topArchetype);
