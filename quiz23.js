...
function initQuiz() {
  ...
  function selectOption(optionIndex, questionType) {
    ...
    // Initialize lastSelectedTime if not set
    if (!window.lastSelectedTime) {
      window.lastSelectedTime = 0;
    }

    if (now - window.lastSelectedTime < DEBOUNCE_DELAY) {
      return;
    }
    window.lastSelectedTime = now;
    ...
    if (currentQuestion.type === 'multi') {
      ...
      if (index === -1) {
        ...
        if (currentQuestion.selectedOptions.length < 2) {
          currentQuestion.selectedOptions.push(optionIndex);
        } else {
          // Remove oldest selection if at limit
          currentQuestion.selectedOptions.shift();
          currentQuestion.selectedOptions.push(optionIndex);
        }
      } else {
    ...
    userResponses[currentQuestionIndex] = [...currentQuestion.selectedOptions];
    ...
    try {
      ...
      setTimeout(() => {
        try {
          ...
          const sortedArchetypes = getSortedScores('archetypes');
          const secondaryArchetype = sortedArchetypes[1] || topArchetype;

          const topSuperpower = getTopScore('superpowers');
          const sortedSuperpowers = getSortedScores('superpowers');
          const secondarySuperpower = sortedSuperpowers[1] || topSuperpower;
          ...
          const resultHTML = `
            <div class="results-container">
              ...
              <div class="superpowers-section">
                ...
                ${secondarySuperpower && secondarySuperpower !== topSuperpower ? `
...
window.addEventListener('error', function(event) {
  ...
  // Add these functions after the selectOption function
  function updateOptionButtons() {
    ...
    options.forEach((button, index) => {
      const isSelected = currentQuestion.selectedOptions && 
                        currentQuestion.selectedOptions.includes(index);
  ...
  function updateNextButton() {
    ...
    if (currentQuestion.type === 'multi') {
      const hasEnoughSelections = currentQuestion.selectedOptions && 
                                currentQuestion.selectedOptions.length === 2;
      nextButton.disabled = !hasEnoughSelections;
      nextButton.setAttribute('aria-disabled', !hasEnoughSelections);
    }
  }
